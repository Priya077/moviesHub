...
export default class Movies extends Component {
    ...
    changeMovies=async()=>{
        ...
        //using api sending request
        const res = await axios.get(`https://api.themoviedb.org/3/movie/550?api_key=25accae9d87d6039f09bde2fb8b1864f&language=en-US&page=${this.state.currPage}`);
        let data = res.data
        // console.log(data);
        this.setState({
            movies:[...data.results]
                        ...
                        <div className="movies-list">
                            {
                                this.state.movies.map((movieObj)=>(
                                    <div className="card movies-card" onMouseEnter={()=>this.setState({hover:movieObj.id})} onMouseLeave={()=>this.setState({hover:''})}>
                                        <div><img src={`https://image.tmdb.org/t/p/original${movieObj.backdrop_path}`}  alt={movieObj.title} className="card-img-top movies-img"/>
